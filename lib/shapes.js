...
class Square extends Shapes{
    render(){
        ...
        <text x="100" y="100" dominant-baseline="middle" text-anchor="middle" fill="${this.textColor}" font-size="100">${this.characters}</text>`;
    }
}
class Triangle extends Shapes{
    render(){
        return `<polygon points="0,200 150,0 300,200" stroke="3" stroke-width="4" fill="${this.color}"></polygon>
        <text x="150" y="125" dominant-baseline="middle" text-anchor="middle" fill="${this.textColor}" font-size="100">${this.characters}</text>`
    }
}
